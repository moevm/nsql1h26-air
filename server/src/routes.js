...
router.post('/auth/register', withRouteContext({ defaultMessage: 'Registration failed' }), async (req, res, next) => {
  try {
...
router.post('/auth/login', withRouteContext({ defaultMessage: 'Login failed', logMessage: 'Login error:' }), async (req, res, next) => {
  try {
...
router.get('/exercises', withRouteContext({ defaultMessage: 'Failed to fetch exercises', logMessage: 'Error fetching exercises:' }), async (req, res, next) => {
  try {
    const {
      page = 1,
      limit = 10,
...
router.get('/exercises/:id', withRouteContext({ defaultMessage: 'Failed to fetch exercise' }), async (req, res, next) => {
  try {
...
router.post('/exercises', authenticateToken, requireAdmin, withRouteContext({ defaultMessage: 'Failed to create exercise', logMessage: 'Error creating exercise:' }), async (req, res, next) => {
  try {
...
router.put('/exercises/:id', authenticateToken, requireAdmin, withRouteContext({ defaultMessage: 'Failed to update exercise' }), async (req, res, next) => {
  try {
    const { title, description, category, difficulty, duration, imageUrl, phases } = req.body;
    const normalizedPhases = Array.isArray(phases)
      ? phases.map((phase, index) => ({
...
router.delete('/exercises/:id', authenticateToken, requireAdmin, withRouteContext({ defaultMessage: 'Failed to delete exercise' }), async (req, res, next) => {
  try {
    const exercises = db.collection('exercises');
...
router.get('/comments', withRouteContext({ defaultMessage: 'Failed to fetch comments', logMessage: 'Error fetching comments:' }), async (req, res, next) => {
  try {
...
router.post('/comments', authenticateToken, withRouteContext({ defaultMessage: 'Failed to create comment' }), async (req, res, next) => {
  try {
    const { exerciseId, text } = req.body;
    const trimmedText = typeof text === 'string' ? text.trim() : '';

    if (!exerciseId || !trimmedText) {
      throw createHttpError(400, 'Exercise ID and comment text are required');
    }

    const exercises = db.collection('exercises');
    try {
      await exercises.document(exerciseId);
    } catch (error) {
      if (error.errorNum === 1202) {
        throw createHttpError(404, 'Exercise not found');
      }
    ...
    const duplicateCursor = await db.query(aql`
      FOR c IN comments
      FILTER c.exerciseId == ${exerciseId}
        AND c.userId == ${req.user.id}
        AND c.text == ${trimmedText}
      LIMIT 1
      RETURN c
    `);
    ...
    if (duplicate) {
      throw createHttpError(409, 'You already posted this comment');
    }
    ...
    res.status(201).json({ id: result._key, exerciseId, text: trimmedText, userId: req.user.id });
  } catch (error) {
    next(error);
  }
});

router.get('/reviews', withRouteContext({ defaultMessage: 'Failed to fetch reviews', logMessage: 'Error fetching reviews:' }), async (req, res, next) => {
  try {
    const { exerciseId, page = 1, limit = 20 } = req.query;
    const offset = (parseInt(page) - 1) * parseInt(limit);
...
router.post('/reviews', authenticateToken, withRouteContext({ defaultMessage: 'Failed to create review' }), async (req, res, next) => {
  try {
    const { exerciseId, rating, text } = req.body;
    const trimmedText = typeof text === 'string' ? text.trim() : '';
    const numericRating = parseInt(rating);

    if (!exerciseId || !trimmedText || Number.isNaN(numericRating)) {
      throw createHttpError(400, 'Exercise ID, rating, and review text are required');
    }
    ...
    try {
      ...
      if (error.errorNum === 1210) {
        throw createHttpError(409, 'You have already reviewed this exercise');
      }

      throw error;
    }

    res.status(201).json({ id: result._key, exerciseId, rating: numericRating, text: trimmedText, userId: req.user.id });
  } catch (error) {
    next(error);
  }
});

router.get('/favorites', authenticateToken, withRouteContext({ defaultMessage: 'Failed to fetch favorites', logMessage: 'Error fetching favorites:' }), async (req, res, next) => {
  try {
    const { page = 1, limit = 20 } = req.query;
    const offset = (parseInt(page) - 1) * parseInt(limit);
    const userDocumentId = `users/${req.user.id}`;

    const cursor = await db.query(aql`
      FOR f IN user_favorites
      FILTER f._from == ${userDocumentId}
      SORT f.createdAt DESC
      LIMIT ${offset}, ${parseInt(limit)}
      LET exercise = DOCUMENT(f._to)
      FILTER exercise != null
      RETURN MERGE(exercise, { favoriteId: f._key })
    `);
...
router.post('/favorites', authenticateToken, withRouteContext({ defaultMessage: 'Failed to add favorite' }), async (req, res, next) => {
  try {
    const { exerciseId } = req.body;
    if (!exerciseId) {
      throw createHttpError(400, 'Exercise ID is required');
    }
    ...
    const favorites = db.collection('user_favorites');
    try {
      await favorites.save({
        _from: `users/${req.user.id}`,
        _to: `exercises/${exerciseId}`,
        createdAt: new Date().toISOString()
      });
    } catch (error) {
      if (error.errorNum === 1210) {
        throw createHttpError(409, 'Exercise already in favorites');
      }

      throw error;
    }

    res.status(201).json({ message: 'Favorite added', exerciseId });
  } catch (error) {
    next(error);
  }
});

router.delete('/favorites/:exerciseId', authenticateToken, withRouteContext({ defaultMessage: 'Failed to remove favorite' }), async (req, res, next) => {
  try {
    const exerciseId = req.params.exerciseId;
    const userDocumentId = `users/${req.user.id}`;
    const exerciseDocumentId = `exercises/${exerciseId}`;

    const cursor = await db.query(aql`
      FOR f IN user_favorites
      FILTER f._from == ${userDocumentId} AND f._to == ${exerciseDocumentId}
      REMOVE f IN user_favorites
      RETURN OLD
    `);
    ...
    res.json({ message: 'Favorite removed', exerciseId });
  } catch (error) {
    next(error);
  }
});

router.get('/users', authenticateToken, requireAdmin, withRouteContext({ defaultMessage: 'Failed to fetch users', logMessage: 'Error fetching users:' }), async (req, res, next) => {
  try {
    const {
      page = 1,
      limit = 10,
...
router.get('/users/:id', authenticateToken, requireAdminOrSelf, withRouteContext({ defaultMessage: 'Failed to fetch user' }), async (req, res, next) => {
  try {
    const users = db.collection('users');
...
router.post('/sessions', authenticateToken, withRouteContext({ defaultMessage: 'Failed to create session' }), async (req, res, next) => {
  try {
...
router.get('/statistics', authenticateToken, withRouteContext({ defaultMessage: 'Failed to fetch statistics', logMessage: 'Error fetching statistics:' }), async (req, res, next) => {
  try {
    const {
      groupByX,
      groupByY,
...
router.get('/export', authenticateToken, requireAdmin, withRouteContext({ defaultMessage: 'Failed to export data', logMessage: 'Error exporting data:' }), async (req, res, next) => {
  try {
    const collections = ['users', 'exercises', 'comments', 'reviews', 'user_sessions', 'statistics'];
    const data = {};
...
router.post('/import', authenticateToken, requireAdmin, withRouteContext({ defaultMessage: 'Failed to import data', logMessage: 'Error importing data:' }), async (req, res, next) => {
  try {
    const data = req.body;
    const collections = ['users', 'exercises', 'comments', 'reviews', 'user_sessions', 'statistics'];
    const edges = ['user_favorites'];
